...
| `CentreContent` | What the centre holds: the row, or exactly one enlarged horizon. A discriminated union, so *never both, never neither* is a property of the type rather than a rule somebody has to remember — and the single-panel answer below the viewport floor is this union forced to `enlarged` rather than a second presentation. Enlargement changes what is shown and never what is computed; the test asserts that by object identity, comparing the array on the canvas with `===` inside the page. |
