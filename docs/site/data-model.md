...
| `presentation.minimumViewportWidthPx`, `minimumViewportHeightPx` | The smallest viewport the workspace lays out in, **measured from the built workspace** rather than chosen, and in CSS pixels — so a large window at 200 % zoom is below it and gets the same answer. Beat 018 rebuilt what the arithmetic is made of: the flanking panes flex, so the floor is the width below which a pane cannot be *read* (`workspace.paneMinimumWidthPx`) and not the width it opens at. That took the floor from 2 038 × 728 to 1 658 × 740, and a 2 000 px monitor from the fallback to the six-panel row. The **width** is what decides which the centre holds: six panels at the minimum panel width need 1 658 px between them, and height is not what makes a row of six unreadable. The height is what a floor is otherwise — below it a pane clips — and its first value, 960, was taller than any browser window a reader has, which put every reader below the floor. The schema refuses a floor that cannot hold every declared horizon at the minimum panel width, and prints the arithmetic. |
| `presentation.referenceViewportWidthPx`, `referenceViewportHeightPx` | The window the workspace is designed for and photographed at, on both axes: 2 560 × 1 440. It may not be below the floor, or every figure in this documentation would have been taken in the fallback presentation. |
| `presentation.controlsWidthPx`, `presentation.detailWidthPx` | What the two flanking panes **open at**, not what they are. Beat 013 fixed them at 390 px in every window, which is how 828 px of its floor became unshrinkable; beat 018's panes are shared down proportionally when a window cannot afford them and never below `workspace.paneMinimumWidthPx`. Selecting something still may not move any pane by a pixel (FR-47), and a pane sized to its contents would — so a pane's width is the reader's, and never its content's. |
| `presentation.workspace` | The workspace's own furniture: the height of the status strip, which is not a pane and cannot be closed; the share of the selection column the provenance tabs open at; the width and height below which a pane cannot be read; the sash between two panes; and where the arrangement is kept, with the version it is written against. Every one of them is furniture, which is the whole reason the arrangement may persist at all — see ADR-0014. |
| `presentation.workspace.skillCurveFraction` | The most of the horizons pane the skill curve may take. FR-45 makes the row the payload and gives it the dominant space, and the curve is an aid to reading the figures the panels already carry — so its share is declared rather than left to be whatever the row does not use. Given the leftover it grew to 698 px of a 1 359 px pane to plot six points, which is taller than the row it annotates. A browser test measures the two rectangles. |
...
| `presentation.strip.heightPx`, `thumbnailWidthPx` | The strip's geometry, declared beside the row's. The height is declared rather than fitted so that a horizon going from unscored to scored cannot move the panel beneath it; the thumbnail width is a *display* size, because the canvas is the grid's own size and the browser scales it — averaging cells into a smaller array would be a quantity computed for a picture. The schema refuses a thumbnail as wide as a panel: that would be a second row. |
| `presentation.centreChromeHeightPx` | What the centre costs beyond one panel's field. The centre's height is worked out from this rather than fitted to its contents, and that is what makes *enlargement replaces the contents of the centre region only* true of the geometry rather than of the intention: a centre sized by what is in it would be one height with the row and another with an enlarged panel, and the scores beneath would move on a click. Measured from the built layout, and held by a browser test that requires the centre's rectangle to be identical either way. |
...
| `CentreContent` | What the centre holds: the row, or exactly one enlarged horizon. A discriminated union, so *never both, never neither* is a property of the type rather than a rule somebody has to remember — and the answer below the width the row needs is this union forced to `enlarged` rather than a second presentation — which beat 018's second pass made true of the whole surface and not only of the centre: there is no second arrangement of the panes below the floor any more, just this union forced. Enlargement changes what is shown and never what is computed; the test asserts that by object identity, comparing the array on the canvas with `===` inside the page. |
