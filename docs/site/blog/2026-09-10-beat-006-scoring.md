...
> **Correction, beat 009.** Every skill figure below was computed with observations the
> forecast could not have had. The analysis at the issue instant was being handed *all* of the
> run's observations, including those taken after it, and beat 009 found and fixed it. The
> corrected figures are worse: the model is roughly no better than persistence. See
> [Beat 009: a forecast that was using measurements it could not have had](2026-09-13-beat-009-two-axes.html).
> The reasoning below stands; the numbers in it do not.
...
![Each panel's score in that panel's own column: the statement verbatim, both references, the brief error beside them, and the provenance one disclosure away](../images/006-score.png)
